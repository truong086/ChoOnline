    ...
    public class DanhGiaService : IDanhGiaService
    {
        ...
        public async Task<PayLoad<DanhgiaDTO>> AddDanhgia(DanhgiaDTO danhgia)
        {
            try
            {
                ...
                var checkProduct = _context.products.Where(x => x.id == danhgia.product_id).FirstOrDefault();
        ...
        public async Task<PayLoad<object>> FindAll(string? name, int page = 1, int pageSize = 10)
        {
            ...
            {
                var total = 0;
                var checkProduct = _context.products.Where(x => x.id == int.Parse(name)).FirstOrDefault();
                if (checkProduct == null)
                {
                ...
                var dataDanhGia = _context.danhGias.Include(p => p.product).Include(a => a.account).Where(x => x.product.Equals(checkProduct)).Select(d => new
                {
                    sao = d.sao,
                    message = d.message,
