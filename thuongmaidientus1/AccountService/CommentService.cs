...
namespace thuongmaidientus1.AccountService
{
    public class CommentService : ICommentService
    {
        ...
        public async Task<PayLoad<CommentDTO>> CreateComment(CommentDTO comment)
        {
            ...
            var checkAccount = _context.accounts.Where(x => x.id == comment.account_id).FirstOrDefault();
            if(checkAccount == null)
            {
                return await Task.FromResult(PayLoad<CommentDTO>.BadRequest());
            }
            var checkProduct = _context.products.Where(x => x.id == comment.product_id).FirstOrDefault();
        ...
        public async Task<PayLoad<CommentDescriptionDTO>> CreateCommentDescription(CommentDescriptionDTO commentDescription)
        {
            try
            {
                if(commentDescription.commentDescription_ID == 0)
                {
                    ...
                    var checkAccount = _context.accounts.Where(x => x.id == commentDescription.account_ID).FirstOrDefault();
                    if (checkAccount == null)
                    {
                ...
                if(commentDescription.comment_ID != 0 && commentDescription.commentDescription_ID > 0)
                {
                    ...
                    var checkAcc = _context.accounts.Where(x => x.id == commentDescription.account_ID).FirstOrDefault();
        ...
        public async Task<PayLoad<object>> FindAll(int idPR, int page, int pageSize)
        {
            try
            {
                page = page < 1 ? 1 : page;
                ...
                var checkPr = _context.products.Where(x => x.id == idPR).FirstOrDefault();
                if (checkPr == null)
                {
                    return await Task.FromResult(PayLoad<object>.NotFound());
                }

                // Cách 1
                var list = new List<CommnentList>();

                var data = _context.comments.Include(p => p.products).Include(a => a.accounts).Where(x => x.products == checkPr).ToList();
                ...
                for (var i = 0; i < data.Count; i++)
                {
                    ...
                    listComment.ids1 = dataItem.id;
                    listComment.message = dataItem.message;
                    listComment.image = dataItem.images;
                    listComment.accountName = dataItem.accounts.username;
                    listComment.avatar = dataItem.accounts.image;
                    listComment.CreatedAt = dataItem.CreatedAt;
					//listComment.images = dataItem.images;

					var commentRep = _context.commentDescriptions.Include(a => a.account).Include(c => c.comment).Where(x => x.comment == dataItem).ToList();
                    if (commentRep != null || commentRep.Any())
                    {
                        ...
                        for (var j = 0; j < commentRep.LongCount(); j++)
                        {
                            ...
                            commentReplist.ids = dataRep.id;
                            commentReplist.messagedescription = dataRep.message;
							commentReplist.account_Name = dataRep.account.username;
							commentReplist.image = dataRep.images;
							commentReplist.avatar = dataRep.account.image;
							commentReplist.CreatedAt = dataRep.CreatedAt;

							var checkRepcomment = _context.commentDescriptions.Include(ar => ar.account).Include(c => c.comment).Include(cmd => cmd.commentDescriptions).Where(x => x.commentDescriptions == dataRep).ToList();
                            if (checkRepcomment.Any() || checkRepcomment != null)
                            {
                                for (var k = 0; k < checkRepcomment.LongCount(); k++)
                                {
                                    var rep = new CommnetRepList();
                                    rep.ids = checkRepcomment[k].id;
                                    rep.messagedescription = checkRepcomment[k].message;
                                    rep.account_Name = checkRepcomment[k].account.username;
                                    rep.image = checkRepcomment[k].images;
                                    rep.avatar = checkRepcomment[k].account.image;
                                    rep.CreatedAt = checkRepcomment[k].CreatedAt;
                            ...
                            listComment.commentDescription.Add(commentReplist);
                ...
                //	return await Task.FromResult(PayLoad<object>.NotFound());
                //}
                //            var list = _context.comments.Include(a => a.accounts).ThenInclude(d => d.Danhgia).Include(p => p.products).Include(x => x.commentDescriptions).ThenInclude(ac => ac.account).Where(x => x.products == checkPr).Select(x => new
                //            {
                //                ids1 = x.id,
                //                message = x.message,
