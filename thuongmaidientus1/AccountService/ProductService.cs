    ...
    public class ProductService : IProductService
    {
        ...
        public async Task<PayLoad<ProductDTO>> EditProduct(int id, ProductDTO productDTO, string? name)
        {
            try
            {
                ...
                checkId.soluong = productDTO.soluong;
        ...
        public async Task<PayLoad<object>> FindAll(string? name, string? account_name, string? type, int page = 1, int pageSize = 10)
        {
            try
            {
                ...
                var data = _dbcontext.products.Include(c => c.Categorys).Include(s => s.Shops).ThenInclude(a => a.account).Where(x => !x.Deleted).OrderByDescending(x => x.id).ToList();
                if (type == "donhang")
                {
                ...
                foreach(var item in data)
                {
                    ...
                    productDTO.soluong = item.soluong;
        ...
        public async Task<PayLoad<int>> TotalTonKho()
        {
            try
            {
                var data = _dbcontext.products.Sum(x => x.soluong);
                return await Task.FromResult(PayLoad<int>.Successfully(data));
            }catch(Exception ex)
            {
                return await Task.FromResult(PayLoad<int>.CreatedFail(ex.Message));
            }
        }
    }
}
